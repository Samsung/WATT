...
def render_function(class_name, func_name, sigs, return_type, non_pointer, copy, operator, constructor, func_scope, call_content=None, const=False, array_attribute=False):
  ...
  args = [('%s' % all_args[i].identifier.name if isinstance(all_args[i], WebIDL.IDLArgument) else 'arg%d' % i) for i in range(max_args)]
  if not constructor:
  ...
  function_description = "/**\n * @%s\n" % (("function %s" % full_name) if not constructor else "constructor")
  ...
  for i in range(max_args):
    ...
    if not legacy_mode and compatible_arg:
      ...
      check_msg = "[CHECK FAILED] %s(%s): " % (full_name, arg_name)
     ...
     # body += "  /* arg%d <%s> [%s] */\n" % (i, arg.type.name, inner)
      ...
      if all_checks and optional: body += "  if(typeof %s !== 'undefined' && %s !== null) {\n  " % (arg_name, arg_name)
      # Special case argument types.
      if arg.type.isNumeric():
        if arg.type.isInteger():
          if all_checks: body += "  assert(typeof %s === 'number' && !isNaN(%s), '%sExpecting <integer>');\n" % (arg_name, arg_name, check_msg)
        else:
          if all_checks: body += "  assert(typeof %s === 'number', '%sExpecting <number>');\n" % (arg_name, check_msg)
        # No transform needed for numbers
      elif arg.type.isBoolean():
        if all_checks: body += "  assert(typeof %s === 'boolean' || (typeof %s === 'number' && !isNaN(%s)), '%sExpecting <boolean>');\n" % (arg_name, arg_name, arg_name, check_msg)
        # No transform needed for booleans
      elif arg.type.isString():
        # Strings can be DOM strings or pointers.
        if all_checks: body += "  assert(typeof %s === 'string' || (%s && typeof %s === 'object' && typeof %s.ptr === 'number'), '%sExpecting <string>');\n" % (arg_name, arg_name, arg_name, arg_name, check_msg)
        do_default = True # legacy path is fast enough for strings.
      elif arg.type.isInterface():
        if all_checks: body += "  assert(typeof %s === 'object' && typeof %s.ptr === 'number', '%sExpecting <pointer>');\n" % (arg_name, arg_name, check_msg)
        if optional:
          body += "  if(typeof %s !== 'undefined' && %s !== null) { %s = %s.ptr };\n" % (arg_name, arg_name, arg_name, arg_name)
        else:
    ...
    if do_default:
      if not (arg.type.isArray() and not array_attribute):
        body += "  if (%s && typeof %s === 'object') %s = %s.ptr;\n" % (arg_name, arg_name, arg_name, arg_name)
        if arg.type.isString():
          body += "  else %s = ensureString(%s);\n" % (arg_name, arg_name)
      else:
        # an array can be received here
        arg_type = arg.type.name
        if arg_type in ['Byte', 'Octet']:
          body += "  if (typeof %s == 'object') { %s = ensureInt8(%s); }\n" % (arg_name, arg_name, arg_name)
        elif arg_type in ['Short', 'UnsignedShort']:
          body += "  if (typeof %s == 'object') { %s = ensureInt16(%s); }\n" % (arg_name, arg_name, arg_name)
        elif arg_type in ['Long', 'UnsignedLong']:
          body += "  if (typeof %s == 'object') { %s = ensureInt32(%s); }\n" % (arg_name, arg_name, arg_name)
        elif arg_type == 'Float':
          body += "  if (typeof %s == 'object') { %s = ensureFloat32(%s); }\n" % (arg_name, arg_name, arg_name)
        elif arg_type == 'Double':
          body += "  if (typeof %s == 'object') { %s = ensureFloat64(%s); }\n" % (arg_name, arg_name, arg_name)
    function_description += " * @param {%s} %s\n" % (type_to_js(arg.type), args[i])


  for i in range(min_args, max_args):
    c_names[i] = 'emscripten_bind_%s_%d' % (bindings_name, i)
    body += '  if (%s === undefined) { %s%s(%s)%s%s }\n' % (arg_name, call_prefix, '_' + c_names[i], ', '.join(pre_arg + args[:i]), call_postfix, '' if 'return ' in call_prefix else '; ' + (cache or ' ') + 'return')
  ...
  if all_checks:
    mid_js += [function_description + ' */\n']
  if not constructor:
    mid_js += ['%s.prototype.%s = ' % (class_name, func_name)]
  mid_js += ['function%s(%s) {\n%s\n};\n' % ((' ' + func_name) if constructor else '', ', '.join(args), body[:-1])]
  if not constructor:
    mid_js += ['%s.prototype[\'%s\'] = %s.prototype.%s;\n' % (class_name, func_name, class_name, func_name)]
