...
define(function (require, exports, module) {
    ...
    function _findPseudoRulesets(lastSelectedElement,refresh){
        if (typeof document.getElementById('htmldesignerIframe').contentWindow.getMatchedCSSRules !== 'function') {
            document.getElementById('htmldesignerIframe').contentWindow.getMatchedCSSRules = (el, css = el.ownerDocument.styleSheets) =>
    ...
    function _findCSSRuleSets(lastSelectedElement,refresh){
        // Polyfill(getMatchedCSSRules): The window.getMatchedCSSRules was removed in chrome 63
        if (typeof document.getElementById("htmldesignerIframe").contentWindow.getMatchedCSSRules !== "function") {
            document.getElementById("htmldesignerIframe").contentWindow.getMatchedCSSRules = (el, css = el.ownerDocument.styleSheets) =>
                [].concat(...[...css].map(s => [...s.cssRules||[]])).filter(r => el.matches(r.selectorText));
        }
